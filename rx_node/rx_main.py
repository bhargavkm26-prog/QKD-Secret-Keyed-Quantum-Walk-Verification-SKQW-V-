...
def start_receiver():
    ...
    try:
        ...
        # 5. SECURITY FIX: send Alice our RAW check-round outcomes.
        # K and its predicted distribution never leave Alice's side.
        check_outcomes = [measured_outcomes[i] for i in range(len(round_types)) if round_types[i] == 'C']
        n_rounds = len(check_outcomes)
        connection.send(json.dumps(check_outcomes).encode('utf-8'))

        print("\n=======================================================")
        print(" 🔍 SECURITY CHECK: awaiting Alice's private verdict")
        print("=======================================================")
        print(f"[+] Sent {n_rounds} raw check-round outcomes to Alice for comparison.")
        print("[+] Bob does NOT know Alice's K-derived prediction, and never will.")

        # 6. Receive Alice's verdict (PASS/FAIL only)
        verdict = connection.recv(1024).decode('utf-8')

        print("\n-------------------------------------------------------")
        if verdict == "BREACH":
            print("🚨 [CRITICAL ALERT] Alice reports the channel is COMPROMISED.")
            print("🚨 Aborting: discarding this session's key material.")
            print("=======================================================\n")
            sys.exit(1)
        else:
            print("✅ [VERIFIED] Alice reports the channel is certified secure.")
            print("=======================================================\n")
